...
//本类支持周立功的致远CAN
namespace CANDevices.CAN.PhysicalInterface
{
    /// <summary>
    /// 周立功的CAN
    /// </summary>
    internal class ZlgUsbCan
    {
        ...
        /// 3.定义CAN控制器状态的数据类型。
        /// </summary>
        unsafe private struct VCI_CAN_STATUS
        {
        ...
        #region CAN C++ DLL库加载
        [DllImport(@"CANDevicesLibrarys\controlcan.dll")]
        ...
        [DllImport(@"CANDevicesLibrarys\controlcan.dll", CharSet = CharSet.Ansi)]
        ...
        internal ZlgUsbCan(int canType, UInt32 deviceIndex)
        {
            m_CanType = canType;
        ...
        /// </summary>
        /// <returns></returns>
        internal int GetCanType()
        {
            return (int)m_CanType;
        }
        ...
        unsafe internal UInt32 InitialCan(uint canIndex, uint baudrate)
        {
            ...
            if ((ZlgCanType)m_CanType == ZlgCanType.PCI5010U || (ZlgCanType)m_CanType == ZlgCanType.PCI5020U || (ZlgCanType)m_CanType == ZlgCanType.USBCAN_E_U || (ZlgCanType)m_CanType == ZlgCanType.USBCAN_2E_U)
            {
                //设置波特率
        ...
        /// 子线程发送CAN数据
        /// </summary>
        unsafe internal UInt32 ASyncSendCanData(ref uint canIndex, CanStandardData canSendData)
        {
        ...
        /// 发送CAN数据
        /// </summary>
        unsafe internal UInt32 SyncSendCanData(ref uint canIndex, ref CanStandardData canSendData)
        {
            //CanDataStc canSendData = new CanDataStc();
            VCI_CAN_OBJ sendobj = new VCI_CAN_OBJ();
            //int nTimeOut = 1500;
